	...
	local reanimate=function()
		...
		local noYvelTime=1
		local lastsine=sine
		-- Lightning state — declared here so mainFunction can close over them
	...
	local function destroyLightning()
		...
		local function spawnLightning()
				...
				-- Each entry: {worldCFrame, sizeX, sizeY, sizeZ}
				local boltData = {
				...
				for idx, data in ipairs(boltData) do
						...
						-- Point this bolt toward the next one, or keep its own rotation for the last
		...
		-- Lightning heartbeat — all variables in same reanimate() scope
    ...
    -- Hat Trail heartbeat
    local hatTrailFadeDuration = 0.6 -- << change this to control fade speed
    local hatTrailQueue = {} -- stores {clone, startTime, startTransparency}
    ...
    tinsert(cons, Connect(heartbeat, function()
        ...
        while i <= #hatTrailQueue do
            ...
            local progress = (now - entry.startTime) / entry.fadeDuration  -- uses per-clone duration
            if progress >= 1 then
				...
				for partTable, partCF in next, cframes do
						...
						if realPart and partTable.m then
								...
								if ok and clone then
										...
										-- Handle both MeshPart and SpecialMesh parts safely
										local mesh = FindFirstChildOfClass(clone, "SpecialMesh")
										pcall(ClearAllChildren, clone) -- pcall in case children are locked
										if mesh then
												pcall(function()
		...
		local attackDuration = 0.45    -- seconds the attack animation runs
		local currentAttackMode = nil  -- which mode's attack is playing
		local currentAttackKey  = nil  -- which key triggered it (e.g. "z")

		-- Attack definitions table
		-- Layout: attacks[modeName][keyName] = function(progress)
		-- progress is 0→1 over attackDuration, then auto-resets
		-- Use it to drive a sin curve or a one-shot lerp weight
		local attacks = {}
		...
		local function fireAttack(modeName, keyName)
				...
				attackActive = true
				attackProgress = 0
				attackDuration = attackDef.duration or 0.45  -- use the attack's own duration
				currentAttackMode = modeName
				currentAttackKey = keyName
		end

	-- Add more modes/keys the same way:
	-- registerAttack("default", "x", function(p) ... end)
	-- registerAttack("f",       "x", function(p) ... end)
	-- ──────────────────────────────────────────────────────────────────
	...
	local function C_c()
      ...
      attacks["f"]["m1"] = {
          duration = 1,
					fn = function()
                        ...
                        AJBackAccessory.C0=cf(0,0,0)*angles(rad(0),rad(0),rad(0))
      ...
      attacks["f"]["q"] = {
          duration = 1,
          trailFadeDuration = 0.6, -- how long each ghost clone takes to fade
          trailStartAlpha = 0.3, -- starting transparency of each clone (0=opaque, 1=invisible)
          trailSpawnRate = 2, -- spawn a ghost every N * 0.1s (1=every frame, 2=every other, etc)
          fn = function()
              -- read config from the attack def itself
              local progress = getAttackProgress()
      ...
      attacks["f"]["r"] = {
		...
		addmode("default", {
			idle = function()
				...
				AJBackAccessory.C0=cf(0,0,0)*angles(rad(0),rad(0),rad(90))
				AJBackAccessory.Part1=getPart("Right Arm") AJBackAccessory.C1=cf_0
			end,
			walk = function()
        ...
        -- ── Keybind system ─────────────────────────────────────────────
				-- Maps KeyCode → {mode, attackKey}
				-- Add entries here to bind more keys in more modes.
				local attackKeybinds = {
						[e.KeyCode.Z] = { modeKey = "f", attackKey = "z" },
						[e.KeyCode.X] = { modeKey = "f", attackKey = "x" },
           	[e.KeyCode.Q] = { modeKey = "f", attackKey = "q" },
						[e.KeyCode.V] = { modeKey = "f", attackKey = "v" },
            [e.KeyCode.R] = { modeKey = "f", attackKey = "r" },
						-- e.g. [e.KeyCode.X] = { modeKey = nil, attackKey = "x" },
				}
				...
				-- modeKey = nil  means "fire for whatever current mode is active"
				-- modeKey = "f"  means "only fire when in mode f"
				...
				_G.AttackInputConnection = UIS.InputBegan:Connect(function(input, gpe)
						...
						if mouseBind then
							...
							if not attacks[targetMode] then
								targetMode = "default"
							end
						...
						-- Determine which mode name to look up
						-- t.getJoint etc. are already captured above via the addmode calls.
						-- We read the active mode from the key-toggle logic in reanimate.
						-- The cleanest way: resolve via the current _G.ModeV string.
						local activeModeStr = tostring(_G.ModeV or "default")

						-- If the bind is mode-locked, enforce it
						if bind.modeKey and bind.modeKey ~= activeModeStr then return end

						-- Check the reanimate table has an attack for this mode
						local targetMode = bind.modeKey or activeModeStr
						if not attacks[targetMode] then
								-- Fallback to default mode attack if this mode has none
								targetMode = "default"
						end
				...
				-- Setup Keybinds
				_G.ModeV = "default" -- Default (matches attacks table keys)

				local keyBinds = {
						[Enum.KeyCode.F] = "f"  -- "f" matches attacks["f"] and addmode("f")
				}
			  ...
			  G2L["1"].ScreenInsets = Enum.ScreenInsets.None
				-- ============================================================
				-- FadeInThing (edge vignette frames, all scale-based)
				-- ============================================================
				...
				-- ============================================================
				-- Text folder (bottom bar labels + divider lines)
				-- ============================================================
				...
				-- Container frame for text labels
				...
				RunService.RenderStepped:Connect(function(dt)
						...
						-- Vertical oscillation  ±0.4% screen height, speed 0.9
						local oscY = math.sin(clock * 0.9) * 0.004

						-- Rotation oscillation  ±5 degrees, speed 0.45 (half of 0.9)
						local rotDeg = math.sin(clock * 0.45) * 2

						-- Move all bottom pieces together vertically
						...
						-- Rotate the container (labels live inside so they rotate too)
						G2L["e"].Rotation = rotDeg

						-- Rotate the divider lines to match
				...
				_G.HatInputConnection = UIS.InputBegan:Connect(function(input, gpe)
						...
						if selectedMode then
								if _G.ModeV == selectedMode then
										_G.ModeV = "default"  -- was "gunless", now "default"
								else
	...
	-- ═══════════════════════════════════════════════════════
	-- HOW TO ADD MORE ATTACKS
	-- ═══════════════════════════════════════════════════════
	--[[
			To add an X attack in default mode:
	
			registerAttack("default", "x", function(p)
					local alpha = sin(p * math.pi)
					LeftShoulder.C0 = Lerp(LeftShoulder.C0,
							cfMul(cf(-1,0.5,0), angles(-(math.pi*0.7*alpha),0,0)),
							deltaTime*4)
			end)
	
			Then add the keybind entry in attackKeybinds:
			[e.KeyCode.X] = { modeKey = nil, attackKey = "x" },
	
			To make an attack only fire in gun mode:
			[e.KeyCode.X] = { modeKey = "gun", attackKey = "x" },
	
			The attack function receives progress (0→1) every frame.
			sin(progress * math.pi) gives a smooth 0→peak→0 envelope.
			Multiply your offset by that alpha to get a clean arc.
	
			attackDuration controls how long the animation takes (seconds).
			Change it per-attack by saving the old value, setting a new one
			before fireAttack(), then restoring it, or make it a field in the
			attack table if you want per-attack durations.
	]]
